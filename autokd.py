...
class AutoKD:
    ...
    def start(self):
        # EDIT THIS TO CHANGE BOARD
        image = 1
        
        path = f"dat/cropped/{image}.jpg"
    ...
    def make_grayscale(self, dom_col_matrix):
        ...
        cv.imshow("Input Image", self.input_img)
        cv.imshow("Dominant Colors", color_matrix_scaled)
        cv.imshow("Dominant Colors (Grayscale)", self.gray_img)
        cv.waitKey(0)
